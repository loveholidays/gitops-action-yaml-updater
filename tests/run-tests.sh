...
# Test 11: HELM_VALUES mode - structured default image without containerName
run_test \
  "HELM_VALUES: Update default structured image without containerName" \
  "HELM_VALUES" \
  "avocado" \
...
# Test 12: HELM_VALUES mode - top-level image tag without repository
run_test \
  "HELM_VALUES: Update default image tag without repository" \
  "HELM_VALUES" \
  "boost-dashboard" \
  "${FIXTURES_DIR}/helm-values-image-tag-only.yaml" \
  "prod-new" \
  "tag: prod-new"
